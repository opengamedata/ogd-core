...
class FeatureRegistry(ExtractorRegistry):
    ...
    def _loadFromSchema(self, schema:GameSchema, loader:ExtractorLoader, overrides:Optional[List[str]]=None):
        ...
        agg_list = list(agg_load_set)
        agg_list.sort(key = lambda x : x.Name)
        for agg_schema in agg_list:
        ...
        per_load_list = list(per_load_set)
        per_load_list.sort(key = lambda x : x.Name)
        for per_schema in per_load_list:
