...
class PopulationExtractor:
    ...
    #                       feature data.
    def __init__(self, ExtractorClass: Type[FeatureLoader], game_schema: GameSchema, feature_overrides:Union[List[str],None]=None, pop_file:IO[str]=sys.stdout):
        ...
        if self._ExtractorClass is LakelandExtractor:
            self._loader = LakelandExtractor(session_id="population", game_schema=self._game_schema, feature_overrides=self._overrides, sessions_file=pop_file)
        else:
            self._loader = self._ExtractorClass(session_id="population", game_schema=self._game_schema, feature_overrides=feature_overrides)
    ...
    def ProcessEvent(self, event:Event):
        ...
        self._loader.ExtractFromEvent(event=event)
    ...
    def CalculateAggregateFeatures(self):
        self._loader.CalculateAggregateFeatures()

    def GetPopulationFeatureNames(self) -> List[str]:
        return self._loader.GetFeatureNames(self._game_schema, overrides=self._overrides) + ["SessionCount"]

    def GetPopulationFeatures(self) -> List[Any]:
        return self._loader.GetFeatureValues() + [len(self._sess_encountered)]
    ...
    def ClearLines(self, pop_file:IO[str]=sys.stdout):
        ...
        if self._ExtractorClass is LakelandExtractor:
            ...
            self._loader = self._ExtractorClass(session_id="population", game_schema=self._game_schema, feature_overrides=self._overrides)