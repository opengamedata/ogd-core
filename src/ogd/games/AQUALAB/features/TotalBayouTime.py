...
class TotalBayouTime(Extractor):
        ...
        if event.app_version == 'Aqualab' or event.app_version == 'None':
            if event.EventSource == EventSource.GAME:
                if self.on:
                    ...
                    if not event.EventData.get('job_name', {}).get('string_value').startswith(self.bayou):
                        self.on = False
                else:
                    if event.EventData.get('job_name', {}).get('string_value').startswith(self.bayou):
                        self.on = True
