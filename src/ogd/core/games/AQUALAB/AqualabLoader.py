...
from ogd.core.games.AQUALAB.detectors import *
from ogd.core.games.AQUALAB.features import *
...
class AqualabLoader(ExtractorLoader):
    ...
    @staticmethod
    def GetAqualabJobCount(db_export_path:Path=Path(".") / "ogd" / "core" / "games" / "AQUALAB"):
        db_export = loadJSONFile(filename="DBExport.json", path=db_export_path)
        return len(db_export.get("jobs", []))

    @staticmethod
    def GetAqualabTaskCount(db_export_path:Path=Path(".") / "ogd" / "core" / "games" / "AQUALAB"):
        db_export = loadJSONFile(filename="DBExport.json", path=db_export_path)
