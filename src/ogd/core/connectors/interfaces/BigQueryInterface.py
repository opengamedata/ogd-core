...
class BigQueryInterface(EventInterface):
    ...
    def _open(self, force_reopen: bool = False) -> bool:
        ...
        if not self._is_open:
            ...
            elif isinstance(self._source_schema.Source, BigQuerySchema):
                os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = self._source_schema.Source.Credential or "NO CREDENTIAL CONFIGURED!" or f"./{self._game_id}.json"
                self._client = bigquery.Client()
            else:
    ...
    def DBPath(self, min_date:Optional[date]=None, max_date:Optional[date]=None) -> str:
        ...
        :rtype: str
        """
        if isinstance(self._source_schema.Source, BigQuerySchema):
            ...
            return f"{self._source_schema.Source.AsConnectionInfo}.{self._source_schema.DatabaseName}.{self._source_schema.TableName}_{date_wildcard}"
        else:
