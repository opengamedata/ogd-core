...
class JobsAttempted(Feature):
    ...
    def _extractFromEvent(self, event:Event) -> None:
        if self._validate_job(event.event_data["job_id"]):
            ...
            elif event.event_name == "complete_job" and self._job_id == self._job_num and user_code == self._user_code:
    ...
    def _validate_job(self, job_data):
        ...
        if job_data['int_value'] is not None:
            ...
            utils.Logger.Log(f"Got invalid job_id data in JobsAttempted", logging.WARNING)
        return ret_val
