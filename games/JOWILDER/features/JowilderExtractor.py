...
class JowilderExtractor(LegacyFeature):
    ...
    def _extractFromEvent(self, event:Event):
        try:
            ...
            Logger.Log(self.get_debug_string(version=event.LogVersion, num_lines=2), logging.DEBUG)
    ...
    def _extractFeaturesFromRow(self, event:Event):
        ...
        # If row is valid, process it.
            ...
            if not self._CLIENT_START_TIME:
                ...
                self.setValByName("version", event.LogVersion)
                ...
                if self.verbose:
                    Logger.Log(f'{"*" * 10} {self._session_id} v{event.LogVersion} @ {self._CLIENT_START_TIME} {"*" * 10}')
            ...
            if event_type_str == "wildcard_click":
                d = event.EventData
                if Event.CompareVersions(event.LogVersion, "4") < 0:
            ...
            if "click" in event_type_str:
                self._extractFromClick(timestamp=event.Timestamp, event_data=event.EventData, version=event.LogVersion)
            ...
            if event_type_str == "checkpoint":
               ...
               self._extractFromQuizquestion(timestamp=event.Timestamp, event_data=event.EventData)
                ...
                self._extractFromStartgame(timestamp=event.Timestamp, event_data=event.EventData, version=event.LogVersion)
                ...
                self._extractFromWildcard_click(timestamp=event.Timestamp, event_data=event.EventData, version=event.LogVersion)
    ...
    def _extractFromClick(self, timestamp:datetime, event_data, version):
        ...
        def new_click():
            ...
            if self.cur_interaction is not None:
                self.feature_cc_inc(self._INT_PREFIX+"total_duration", self.cur_interaction, time_between_clicks.total_seconds())
                if not self.finished_encounters[self.cur_interaction]:
                    self.feature_inc("first_enc_duration", time_between_clicks.total_seconds(), interaction_num=self.cur_interaction)
            if self.cur_objective is not None:
                self.feature_cc_inc(self._OBJ_PREFIX+"time_to_next_obj", index=self.cur_objective, increment=time_between_clicks.total_seconds())
            self.feature_average('avg_time_between_clicks', time_between_clicks)

            if time_between_clicks < self._IDLE_THRESH:
                self.feature_inc('time_active_clicking', time_between_clicks.total_seconds())
        ...
        if self.last_display_time_text: # if the previous log had text
            ...
            if _text == last_text:
                Logger.Log(f"The player read {last_text} twice in a row!", logging.WARNING)
            else:
    ...
    def _extractFromWildcard_hover(self, timestamp, event_data):
        ...
        if self.cur_question in [10,16] and self.chosen_answer and _interacted_fqid:
            Logger.Log(f'During Jowilder Q{self.cur_question}, player chose {self.chosen_answer} '
