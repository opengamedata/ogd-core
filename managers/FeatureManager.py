...
class FeatureManager:
    ...
    def _try_update(self, as_str:bool = False):
        if not self._up_to_date:
            ...
            list_o_lists   : List[List[ExportRow]] = [[session.Lines for session_id,session in session_list.items() if (session_id != "null" or self._used_null_sess[player_name])] for player_name,session_list in self._sessions.items()] if self._sessions is not None else []
            ...
            self._latest_values = {
                ...
                "players"    : [player.Lines for player_id,player in self._players.items() if (player_id != "null" or self._used_null_play)] if self._players is not None else [],
