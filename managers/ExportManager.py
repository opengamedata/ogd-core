    ...
    def ExecuteRequest(self, request:Request) -> RequestResult:
        ...
        try:
            ...
            if load_class is not None:
                self._setupManagers(request=request, load_class=load_class, feature_overrides=request._feat_overrides)
            ...
            self._processSlices(request=request, ids=_sess_ids, slices=_session_slices)
    ...
    def _setupManagers(self, request:Request, load_class:Type[ExtractorLoader], feature_overrides:Optional[List[str]]):
        ...
        if request.ExportSessions or request.ExportPlayers or request.ExportPopulation:
            self._feat_mgr = FeatureManager(LoaderClass=load_class, exp_modes=request._exports,
                                            game_schema=_game_schema, feature_overrides=feature_overrides)
        else:
