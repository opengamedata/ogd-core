...
class ExportManager:
    ...
    def ExecuteRequest(self, request:Request, feature_overrides:Union[List[str],None]=None) -> Dict[str,Any]:
        ...
        _table_schema = TableSchema(schema_name=f"{_table_name}.json")
        ...
        try:
            ...
            if request.ToFile():
                ...
                file_manager = FileManager(exporter_files=request._exports, game_id=_game_id, data_dir=_data_dir, 
                                            date_range=request._range.GetDateRange(),      extension="tsv")
                file_manager.OpenFiles()
                self._setupFileHeaders(request=request, file_manager=file_manager)
                ret_val['success'] = self._executeFileRequest(request=request, file_manager=file_manager, feature_data=_result)
                self._setupReadme(file_manager=file_manager, game_schema=_game_schema, table_schema=_table_schema)
    ...
    def _executeDataRequest(self, request:Request, table_schema:TableSchema) -> Dict[str,Any]:
        ...
        session_slices = [[sess_ids[i] for i in range( j*_slice_size, min((j+1)*_slice_size, _num_sess) )]
    ...
    def _executeFileRequest(self, request:Request, file_manager:FileManager, feature_data:Dict[str,Any], ) -> bool:
        ret_val  : bool = False
